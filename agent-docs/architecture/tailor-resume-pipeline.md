...
- Model-backed Tailor Resume requests have explicit output-token caps and a five-minute timeout per request attempt. Step 1 falls back to deterministic keyword hints after a model timeout; later generation stages emit failed step events and stop retrying that attempt path when a timeout occurs.
