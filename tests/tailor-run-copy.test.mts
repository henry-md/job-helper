...
test("buildCompletedTailoringMessage humanizes the same-line-count page-fit warning", () => {
  assert.equal(
    buildCompletedTailoringMessage({
      jobLabel: "Software Engineer at Microsoft",
      tailoredResumeError:
    ...
    "Saved a tailored draft for Software Engineer at Microsoft, but the page-fit check couldn't reduce the page size.",
  );
});
