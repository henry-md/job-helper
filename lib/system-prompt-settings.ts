...
function buildTailorResumeInterviewToolContractBlock() {
  return (
    ...
    "2. The tool call is the control-plane output and its assistantMessage or completionMessage is rendered visibly in the chat UI, so do not reiterate things you put there.\n" +
    "3. initiate_tailor_resume_probing_questions is presentation-only. Put the concise question text in assistantMessage. Do not include USER.md edits, learnings, keyword decisions, non-technology terms, or debug fields in this tool.\n" +
    ...
    "17. Never put placeholders such as \"... rest unchanged\" or \"[existing content]\" inside USER.md edit fields.\n"
  );
}
...
const defaultSystemPromptSettings = {
  ...
  tailorResumePlanning:
    ...
    "8. When USER.md contains quoted experience evidence for a job-emphasized technology, strongly consider targeting the matching experience bullet instead of only editing the technical skills section.\n" +
    "9. Because implementation is block-scoped, plan new bullet-shaped evidence as a replacement/swap for an existing lower-signal bullet in the same experience whenever possible. Use an empty desiredPlainText only when deleting one whole bullet or line is the intended edit.\n" +
    "10. Your primary goal is to make sure the final planned resume text includes every remaining high-priority keyword that is already supported by the original resume, USER.md, or Step 2 user-confirmed learnings. Make any additional improvements after that coverage obligation is satisfied.\n" +
    "11. When editing Skills or Technical Skills, add only actual skills. A new skills entry must be a concrete tool, language, framework, database, infrastructure tool, developer tool, or named method, and it must either already be supported as a real skill by the source resume or have a dedicated USER.md sentence/bullet for that exact technology saying it can be listed in skills, describing direct experience/exposure, or providing quoted experience evidence. If any planned non-skills bullet uses a concrete actual skill that passes this gate and a Skills or Technical Skills block is editable, return a Skills change in the same plan that lists that exact skill in the closest existing category. Do not rely on the bullet alone to cover the Skills section. Do not add peppering/capability phrases such as RESTful, RESTful APIs, cloud infrastructure, data structures, production infrastructure, or similar wording to Skills merely because the job description emphasized them; use those in bullets or coverage checks when they fit naturally. Skills-only concrete tools like Windsurf are valid Skills additions when USER.md has a dedicated note permitting them, even if they are not worthy of an experience bullet.\n" +
    "12. Follow the Available tools section before final JSON. Use low-priority terms when they fit truthfully and naturally, but do not let them crowd out the high-priority coverage goal.\n\n" +
    "Metadata rules:\n" +
  ...
  tailorResumeInterview:
    ...
    "4. Ask direct questions in assistantMessage only.\n" +
    "5. As soon as you gather all context needed, call finish_tailor_resume_interview tool. There is no need to confirm this with the user over text — as soon as the tool is invoked, they will see a modal that they can accept to end the chat. Do not waste the user's time asking permissions to edit user.md or ending the chat. \n" +
    "6. Be concise. Avoid throat-clearing like \"I have a few questions,\" \"this would strengthen the resume,\" or \"I'm trying to clarify\".\n" +
    "7. When finishing, learnings must be a compact working summary for the next model stage, not a transcript dump. Only include details grounded in the user's answers or directly restated from the accepted plan.\n" +
    "8. If an accepted plan is present, every learning.targetSegmentIds entry must reference only segmentIds from that plan. If Step 3 planning has not run yet or no target segment is certain, use an empty targetSegmentIds array and describe the likely experience/project in detail.\n" +
    "9. Step 2 records confirmed support and explicit rejections; it does not need to prove every job keyword is usable before finishing. If a high-priority keyword is still unsupported after the user's answer, leave it unsupported so Step 3 does not invent experience.\n" +
  ...
  tailorResumeImplementation:
    ...
    "19. Follow the Available tools section before final JSON.\n\n" +
    "Common pitfalls:\n" +
