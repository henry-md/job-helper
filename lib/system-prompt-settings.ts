...
function buildTailorResumeInterviewDebugBlock(input: {
  ...
  return (
    ...
    "3. The app records that the question was debug-forced outside the tool arguments; do not add debug-only fields to any interview tool.\n\n"
  );
}
...
function buildTailorResumeInterviewToolContractBlock() {
  return (
    ...
    "2. The tool call is the control-plane output and its assistantMessage, completionMessage, and technologyContexts are rendered visibly in the chat UI, so do not reiterate things you put there.\n" +
    "3. initiate_tailor_resume_probing_questions is presentation-only. Put the concise question text in assistantMessage and optional visible cards in technologyContexts. Never repeat definitions, examples, or technologyContexts content outside the tool call. Do not include USER.md edits, learnings, keyword decisions, non-technology terms, or debug fields in this tool.\n" +
    "4. finish_tailor_resume_interview is the only tool that writes USER.md. Put a short status in completionMessage, compact final learnings in learnings, and all durable memory updates in userMarkdownEditOperations. The app will immediately save the memory updates and continue tailoring; do not ask the user to confirm, press Done, or keep chatting before the tool can finish. Never ask permission to update USER.md after the user answers; updating USER.md is the point of this chat.\n" +
    ...
    "6. If the latest user answer asks you for a sample bullet, example, draft, clarification, or review, keep the chat open with initiate_tailor_resume_probing_questions only when that reply is still needed to capture accurate memory. For technology bullet examples, use technologyContexts so the app renders them as collapsible cards; keep assistantMessage concise. For non-technology replies, answer directly in assistantMessage only if more detail is still needed. If the user also asks to end or proceed, save the durable facts and finish. Do not ask whether to add, replace, append, insert, or swap resume bullets; that placement decision belongs to Step 3 planning.\n" +
    "7. Use finish_tailor_resume_interview after an interview has already started as soon as the latest user answer gives enough durable context, or whenever the user asks to end, finish, wrap up, proceed, or start tailoring. The tool call ends the chat and starts the remaining tailoring steps immediately.\n" +
    "7a. If the user gives quoted bullet text and names the employer, project, section, or skills-only constraint after the quote with phrasing like \"in Johns Hopkins\", \"for N-body\", \"under Kubernetes too\", or \"skills only\", treat that as an explicit mapping. Save it to USER.md and finish; do not ask whether to add the confirmed bullets or where they should go.\n" +
    "8. update_tailor_resume_non_technologies is the only tool that removes rejected keywords or updates the durable non-technology deny-list. Use it when the latest user answer says scraped terms are not real technologies, not real skills, nonsense, or should not be included. Put every rejected emphasized keyword in nonTechnologyTerms, set matching keywordDecisions entries to action \"remove\", and say that you are updating the non-technology list.\n" +
    "9. skip_tailor_resume_interview takes only a reason and is allowed only on the first turn when no interview should start at all. Do not write assistant text or mutations for skip.\n" +
    "10. If the user answer added durable facts but one more concrete follow-up is genuinely required, ask the follow-up with initiate_tailor_resume_probing_questions and then write all durable facts from the chat when you later call finish_tailor_resume_interview.\n" +
    ...
    "18. Never put placeholders such as \"... rest unchanged\" or \"[existing content]\" inside USER.md edit fields.\n"
  );
}
...
function buildTailorResumePlanningOutputContractBlock() {
  return (
    ...
    "4. Use emphasizedTechnologies only for resume-searchable skills a realistic candidate could add to a resume Skills or Technical Skills section: concrete technologies, programming languages, named frameworks, named libraries, databases, infrastructure tools, developer tools, and named technical methods.\n" +
    "5. Do not extract general domains, responsibilities, processes, or environment descriptions that are not concrete skills. For example, never return Production Infrastructure or Production clusters.\n" +
    "6. Shorten compound job phrases to the core resume skill. For example, return Kubernetes for Kubernetes-based PaaS, not Kubernetes-based PaaS.\n" +
    "7. Do not extract company-specific internal products, product suites, platform names, customer-facing product brands, team names, mission names, or nouns that only describe what the company builds. Skip employer-branded terms unless the posting explicitly asks candidates to have prior experience using that product.\n" +
    "8. Infer priority from the posting: high for required/basic/minimum terms, repeated terms, title/team-defining terms, and strongly emphasized preferred terms; low for weaker preferred, nice-to-have, incidental, or broad ecosystem terms.\n" +
    "9. Return one atomic technology per item. Hard rule: if a slash, comma, parenthetical, or grouped phrase separates two distinct technologies, you must return separate emphasizedTechnologies items for each named technology, such as TypeScript and JavaScript instead of TypeScript/JavaScript, and React and Next.js instead of React / Next.js.\n" +
    "10. Do not replace named skills with broad group labels when the posting names specific technologies; include the specific names.\n" +
    "11. Prefer the core technology term and remove interchangeable vendor or marketing fluff when the inner term is what people actually list on resumes. For example, return Visual Studio instead of Microsoft Visual Studio. We need the smaller stable term so deterministic string matching can find it in resumes.\n" +
    "12. Do not return workflow nouns, UI/product feature labels, roadmap terms, or generic system categories such as commit previews, blueprints, storage systems, frontend frameworks, developer experience, internationalization, or internet terminology unless the posting explicitly frames that exact phrase as a candidate skill and no named concrete technology is present.\n" +
    "13. Do not return browser/project names such as Chromium merely because the employer builds on or for them; include them only when the posting asks for candidate experience with that technology.\n" +
    "14. Do not return section-label fields. Keep section/source context only in evidence.\n"
  );
}
...
function buildTailorResumeInterviewTechnologyContextBlock() {
  return (
    ...
    "The input includes deterministic keyword presence for the original resume and USER.md. Use that model-only context as the main Step 2 clarification decision anchor after Step 1 keyword scraping and before Step 3 planning runs. It is okay to tell the user that a skill was not found in the resume or USER.md, but do not expose raw analysis fields. Strongly prefer asking when a concrete job technology is missing from both the original resume and USER.md and you cannot cleanly assume the user's experience yourself. Skip questions for vague, generic, or low-value terms such as broad practices, collaboration traits, or phrases like internet terminology. Include low-priority missing terms in the grouped question only when they are concrete and share an obvious likely insertion point with stronger terms. Group ask-worthy missing technologies into one pointed question, such as \"Do you have experience with Go, Cassandra, or Spark?\" and give each technology a one-sentence explanation plus two structured resume examples that include the exact keyword. Keep example bullets inside the per-technology sections, not in the intro. Each examples item must be an object with `text` and `kind`; set kind to `existing` when the suggestion is a slight modification of an existing resume or USER.md bullet, and `new` when it is an entirely new bullet suggestion. The server deterministically rejects any example whose sentence before the `-- <placement>` suffix does not contain the exact technology name from that card. For concrete actual skill keywords that could go in Skills, provide one existing and one new example by default. For softer capability keywords that should not go in Skills, such as RESTful, RESTful APIs, API development, distributed systems, system architecture, cloud infrastructure, data structures, algorithms, microservices, AI systems, or geospatial, provide only existing examples. Example bullets must be ideal FAANG-level resume bullets: concise action + technical scope + positive result in the same sentence, preferably with a metric. Every example bullet text should end with a dash suffix naming a specific company or internship already present in the user's resume, such as `-- NewForm AI`, `-- KnoWhiz`, or `-- Johns Hopkins University`. The suffix means this bullet would be for that resume entry. Never use job-posting product, team, platform, project, technical-category, or technology names as the suffix. If relevance is uncertain, choose the closest existing resume company/internship rather than inventing a product or project suffix.\n"
  );
}
...
const defaultSystemPromptSettings = {
  ...
  tailorResumePlanning:
    ...
    "1. Return emphasizedTechnologies as a deduped list of resume-searchable skills the job description emphasizes. Include only terms a realistic candidate could add to a resume Skills or Technical Skills section: concrete technologies, programming languages, named frameworks, named libraries, databases, infrastructure tools, developer tools, and named technical methods.\n" +
    ...
    "5. Never return Production Infrastructure or Production clusters. Shorten compound job phrases to the core resume skill: return Kubernetes for Kubernetes-based PaaS, not Kubernetes-based PaaS.\n" +
    "6. Preserve exact core technology names and capitalization when possible.\n" +
    "7. Mark priority high for required/basic/minimum technologies, repeated technologies, title/team-defining technologies, and unusually strong preferred signals. Mark priority low for weaker preferred, nice-to-have, incidental, or broad ecosystem terms.\n" +
    "8. Return one atomic technology per item. Hard rule: if a slash, comma, parenthetical, or grouped phrase separates distinct technologies, you must return separate emphasizedTechnologies items for each named technology, such as TypeScript and JavaScript instead of TypeScript/JavaScript, React and Next.js instead of React / Next.js, and Python, Java, C++ as three items. Do not use a broad wrapper label such as front-end frameworks when the posting names specific frameworks.\n" +
    "9. Prefer the core technology term and remove interchangeable vendor or marketing fluff when the inner term is what people actually list on resumes. For example, return Visual Studio instead of Microsoft Visual Studio. We need the smaller stable term so deterministic string matching can find it in resumes.\n" +
    "10. Do not return broad group labels or generic categories such as frontend frameworks, storage systems, platform, infrastructure, developer experience, production infrastructure, production clusters, commit previews, blueprints, internationalization, or internet terminology when they are product/domain language rather than named candidate skills.\n" +
    "11. Do not return browser/project names such as Chromium merely because the employer builds on or for them; include them only when the posting asks for candidate experience with that technology.\n" +
    "12. When choosing planned changes, include high-priority technology keywords when the resume, USER.md, Step 2 user-confirmed learnings, or existing block text already supports them. Do not invent unconfirmed technology experience.\n" +
    "13. The high-priority list you receive after Step 2 should already have bad keywords removed and should already be accounted for in either the original resume or USER.md. Your job in Step 3 is to move those grounded keywords into the tailored resume text itself.\n" +
  ...
  tailorResumeInterview:
    ...
    "2. Only ask when the keyword cannot already be found in the resume or USER.md.\n" +
    "3. On the first ask turn, ask all useful missing-technology questions together in one grouped message. Do not ask one technology per turn.\n" +
    "4. Do not put technology definitions or example bullets in assistantMessage. Put them in technologyContexts! And DEFINITELY do not put them in both places — this would be redundant, since the user can see both. technologyContexts will be rendered in the chat.\n" +
    "5. Each technologyContexts entry must contain the technology name, one sentence explaining what the technology is and which resume activities it most likely maps to, then different one-sentence example resume bullets or bullet fragments that include the exact keyword. Return exactly two examples by default. If the user asks for more, give more. Each example item must be an object with `text` and `kind`; set kind to `existing` when the suggestion is a slight modification of an existing resume or USER.md bullet, and `new` when it is an entirely new bullet suggestion. The server deterministically rejects any example whose sentence before the `-- <placement>` suffix does not contain the exact technology name from that card. For concrete actual skill keywords that could go in the Skills section of a resume, provide one existing and one new example by default. For softer capability keywords that would not go in the Skills section of a resume (like RESTful, RESTful APIs, API development, distributed systems, system architecture, cloud infrastructure, data structures, algorithms, microservices, AI systems, or geospatial) provide only 'existing' type examples. Examples must be ideal FAANG-level resume bullet suggestions: concise action + technical scope + positive result in the same sentence, preferably with a metric. Avoid generic low-signal examples like \"Used Spark for analytics\" or \"Built Cassandra tables\". Every example bullet text should end with a dash suffix naming a specific company or internship already present in the user's resume. You must look at their actual resume and choose one of their employers. The suffix means this bullet would be for that resume entry. Never use anything else, like a technical-category, as the suffix. If relevance is uncertain, choose the closest existing resume company/internship rather than inventing a product or project suffix.\n" +
    "6. Good technology definition style: \"Apache Spark helps you process large amounts of data by splitting it across computers in parallel; common for processing tons of logs, training ML models at scale.\" This is a good model for explaining what the tool is and helping the user recognize adjacent experience.\n" +
    "7. Good first-turn pattern:\n" +
    "\"assistantMessage\": \"I'd like more context on these:\"\n" +
    "\"technologyContexts\": [{ \"name\": \"Cassandra\", \"definition\": \"Cassandra is a distributed NoSQL database used for high-write, large-scale time-series or event data; resume activities map to data modeling, migrations, or cluster tuning.\", \"examples\": [{ \"text\": \"Migrated time-series data from MongoDB to Cassandra to reduce storage costs 40% and improve query tail latency by 2x -- NewForm AI\", \"kind\": \"existing\" }, { \"text\": \"Redesigned Cassandra partition keys for ad-event ingestion to sustain 12k writes/sec while cutting hot-partition retries 35% -- KnoWhiz\", \"kind\": \"new\" }] }, { \"name\": \"Distributed systems\", \"definition\": \"Distributed systems coordinate work across multiple machines; resume activities map to services, data stores, queues, or reliability patterns.\", \"examples\": [{ \"text\": \"Rearchitected distributed import workflow with Java Spring Boot batching and two-phase commits to cut large import failures from ~50% to <4% -- KnoWhiz\", \"kind\": \"existing\" }, { \"text\": \"Refactored distributed API endpoints and inference services for platform-agnostic ad workflows, supporting $50K+/mo in TikTok ad spend -- NewForm AI\", \"kind\": \"existing\" }] }]\n" +
    "8. As soon as you gather all context needed, call finish_tailor_resume_interview tool. There is no need to confirm this with the user over text — as soon as the tool is invoked, they will see a modal that they can accept to end the chat. Do not waste the user's time asking permissions to edit user.md or ending the chat. \n" +
    "9. Be concise. Avoid throat-clearing like \"I have a few questions,\" \"this would strengthen the resume,\" or \"I'm trying to clarify\".\n" +
    "10. If you give technology bullet examples after the first turn, put them in technologyContexts rather than inline assistantMessage text. Again, it's rendered in the UI. \n" +
    "11. When finishing, learnings must be a compact working summary for the next model stage, not a transcript dump. Only include details grounded in the user's answers or directly restated from the accepted plan.\n" +
    "12. If an accepted plan is present, every learning.targetSegmentIds entry must reference only segmentIds from that plan. If Step 3 planning has not run yet or no target segment is certain, use an empty targetSegmentIds array and describe the likely experience/project in detail.\n" +
    "13. Step 2 records confirmed support and explicit rejections; it does not need to prove every job keyword is usable before finishing. If a high-priority keyword is still unsupported after the user's answer, leave it unsupported so Step 3 does not invent experience.\n" +
    "14. If no questions are worth asking on the first turn, call skip_tailor_resume_interview instead of starting a chat.\n" +
    "USER.md memory rules:\n" +
    "1. The current USER.md memory is provided in the input. USER.md was compiled from previous chats just like this one, so do not ask a question if you can get the answer from USER.md.\n" +
    "2. No need to ask the user's permission to edit USER.md. The assumption is that you do this automatically.\n" +
    "3. Prefer append for ordinary new memory. Use exact-match operations only when deduplicating or restructuring existing USER.md content.\n" +
    ...
    "5. Exact user-confirmed experience-evidence bullets must be wrapped in double quotes and end with `-- ExperienceName`, where ExperienceName is the employer, project, or organization the bullet belongs to. These quoted bullets are grounded context for Step 3 planning. They are not a Step 2 decision to add, replace, append, insert, or swap a resume bullet.\n" +
    "6. Non-exact notes must not be quoted. Use unquoted bullets for no-experience notes, skills-only permissions, adjacent exposure, uncertainty, or constraints. Do not make adjacency sound like production experience.\n" +
    "7. If the user confirms several technologies in one answer, write one USER.md bullet or technology section per confirmed technology. Do not collapse them into a generic storage, backend, frontend, or tooling note.\n" +
    "8. For confirmed missing technologies, prefer recording a skills-section category even when you also record experience-evidence bullets, so Step 3 has a direct place to consider the exact keyword. If you are unsure about resume placement but the user confirmed the technology, record that it can be listed in the skills section and finish.\n" +
    "9. Optimistically infer likely employer/project context from the resume, but only record confirmed user experience or confirmed non-experience. Do not ask the user to choose resume edit placement; Step 3 planning owns that decision.\n" +
    "10. When calling finish_tailor_resume_interview, include USER.md edit operations for every durable fact from the entire chat that is not already reflected in the current USER.md. Do not write only the latest user message if earlier answers in the same chat confirmed other technologies or constraints.\n",
  tailorResumeImplementation:
    ...
    "18. If the accepted plan adds actual skills to a skills section, preserve those planned skills entries. When the accepted plan uses a concrete actual skill in a bullet and also includes a Skills or Technical Skills replacement, make sure that replacement lists the same exact skill in the closest existing category. Do not treat the bullet mention as a substitute for the skills-list entry. Do not add extra capability phrases such as RESTful, RESTful APIs, cloud infrastructure, or data structures to Skills merely to improve keyword coverage; they belong in bullets only when the accepted plan already uses them there.\n" +
    "19. Before you return the final JSON implementation, call the check_implemented_resume_keyword_coverage tool on your current LaTeX replacements. If the tool reports missing high-priority keywords, revise the implementation and call the tool again before finalizing.\n\n" +
    "Common pitfalls:\n" +
