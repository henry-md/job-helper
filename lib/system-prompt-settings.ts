...
function buildTailorResumePlanningTechnologyContextBlock() {
  return (
    ...
    "6. The implementation stage is segment-safe and normally replaces one existing block at a time. To add a new bullet-shaped experience from USER.md, plan it as a swap: choose the weakest or least job-relevant existing bullet in the same experience and set desiredPlainText to the adapted quoted USER.md bullet (per rule 3). Do not ask Step 4 to invent a brand-new neighboring bullet outside the targeted segment.\n" +
    "7. If an existing bullet is low-value for this role and should simply disappear, you may plan deletion by setting desiredPlainText to an empty string for that single bullet segment. Use this only for an entire bullet/line segment, not for structural wrappers.\n" +
    "8. Unquoted USER.md notes are constraints, skills-only permissions, adjacency notes, or no-experience statements. Use them conservatively; do not convert adjacency or skills-only permission into an experience bullet unless the note explicitly supports that claim.\n"
  );
}
