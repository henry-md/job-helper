...
function buildTailorResumeInterviewToolContractBlock() {
  return (
    ...
    "8. Use update_tailor_resume_non_technologies when the latest user answer says the scraped terms are not real technologies, not real skills, nonsense, or should not be included. Put every rejected emphasized keyword in nonTechnologyTerms, set matching keywordDecisions entries to action \"remove\", and say that you are updating the non-technology list.\n" +
    "9. Use skip_tailor_resume_interview only on the first turn when no interview should start at all, and do not write assistant text for skip.\n" +
    "10. initiate_tailor_resume_probing_questions may edit USER.md only after the user has answered and only for durable facts that answer added. Use an empty userMarkdownEditOperations array for the first ask turn and for any ask turn that does not add durable memory. skip_tailor_resume_interview must always use an empty userMarkdownEditOperations array.\n" +
    "11. If userMarkdownEditOperations is non-empty, the user-facing assistantMessage or completionMessage must explicitly say that you are updating USER.md.\n" +
    ...
    "15. keywordDecisions is the deterministic place to keep or remove emphasized keywords after the user answers. Use action \"remove\" only when the user explicitly says a keyword is not a real requirement keyword, is nonsense, or should not count for this role.\n" +
...
function buildTailorResumeImplementationTechnologyContextBlock() {
  return (
    ...
    "Quoted bullets under technology headings are user-confirmed experience evidence tied to the experience after `-- ExperienceName`; treat them as grounded starting points that Step 3 planning may adapt into skills entries or block edits. They are not placement instructions by themselves. If the accepted plan uses a quoted USER.md bullet as experience evidence, implement that planned block edit as an actual experience-bullet replacement rather than downgrading it to a skills-only keyword. If the accepted plan includes a skills-section change for supported emphasized technologies, preserve the planned skills keywords unless the plan intentionally used light discretion to keep a capability phrase such as RESTful APIs out of the skills list. Do not drop a keyword just because it also appears in an experience sentence. Unquoted bullets are factual notes, constraints, skills-only permissions, adjacency notes, or no-experience statements; do not turn them into exact resume claims unless the note explicitly supports that claim.\n\n" +
...
function buildTailorResumePlanningTechnologyContextBlock() {
  return (
    ...
    "5. If no quoted bullet exists for the targeted experience but only an unquoted note (skills-only permission, adjacency, no-experience), do not write a fabricated production-style bullet. Either add the keyword to the skills section, target a different experience that does have a quoted bullet, or skip the bullet edit entirely.\n" +
    "6. Skills-section coverage is strongly encouraged when an editable Skills or Technical Skills block exists: include extracted technology keywords as skills whenever they read naturally as skills-list entries and are supported by the original resume or USER.md. Include low-priority extracted technology keywords too unless USER.md explicitly says the user has no experience, no exposure, or does not want the keyword included. Use light discretion for capability phrases that are useful extracted keywords but awkward standalone skills entries; for example, RESTful APIs may belong in a bullet or ATS coverage check without needing to appear as a tool in Skills. Put included keywords under the closest existing category and keep unsupported, no-experience, or explicitly rejected keywords out.\n" +
    "7. The implementation stage is segment-safe and normally replaces one existing block at a time. To add a new bullet-shaped experience from USER.md, plan it as a swap: choose the weakest or least job-relevant existing bullet in the same experience and set desiredPlainText to the adapted quoted USER.md bullet (per rule 3). Do not ask Step 4 to invent a brand-new neighboring bullet outside the targeted segment.\n" +
    "8. If an existing bullet is low-value for this role and should simply disappear, you may plan deletion by setting desiredPlainText to an empty string for that single bullet segment. Use this only for an entire bullet/line segment, not for structural wrappers.\n" +
    "9. Unquoted USER.md notes are constraints, skills-only permissions, adjacency notes, or no-experience statements. Use them conservatively; do not convert adjacency or skills-only permission into an experience bullet unless the note explicitly supports that claim.\n"
  );
}
...
const defaultSystemPromptSettings = {
  ...
  tailorResumePlanning:
    ...
    "12. When an editable skills section exists, highly encourage a skills-section update that includes extracted technology keywords with evidence of user experience in the original resume or USER.md, including low-priority extracted technology keywords unless USER.md explicitly says the user has no experience, no exposure, or does not want the keyword included. Use light discretion for capability phrases that are useful extracted keywords but awkward standalone skills entries; for example, RESTful APIs may be better in a bullet or keyword coverage than listed as a tool in Skills. This is in addition to any experience-bullet sentence that mentions the same technology.\n" +
    ...
    "14. For any extracted technology keyword already supported by the resume or USER.md user experience, strongly prefer carrying the exact keyword into the skills section rather than relying only on a sentence or bullet mention elsewhere in the resume. Low-priority extracted technology keywords should also usually go into skills unless USER.md explicitly records no experience, no exposure, or a do-not-include constraint for that keyword, or the term is a capability phrase like RESTful APIs that reads better in resume text than as a skills-list tool.\n\n" +
    "Reason rules:\n" +
  ...
  tailorResumeImplementation:
    ...
    "18. If the accepted plan adds supported keywords to a skills section, preserve every one of those planned skills keywords in the skills-section replacement.\n" +
