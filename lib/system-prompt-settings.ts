...
const defaultSystemPromptSettings = {
  ...
  tailorResumeInterview:
    ...
    "\"technologyContexts\": [{ \"name\": \"Cassandra\", \"definition\": \"Cassandra is a distributed NoSQL database used for high-write, large-scale time-series or event data; resume activities map to data modeling, migrations, or cluster tuning.\", \"examples\": [\"Migrated time-series data from MongoDB to Cassandra to reduce storage costs 40% and improve query tail latency by 2x -- NewForm AI\", \"Redesigned Cassandra partition keys for ad-event ingestion to sustain 12k writes/sec while cutting hot-partition retries 35% -- KnoWhiz\"] }, { \"name\": \"Spark\", \"definition\": \"Apache Spark helps you process large amounts of data by splitting it across computers in parallel; common for processing logs, event streams, and training data at scale.\", \"examples\": [\"Wrote Spark streaming pipeline to aggregate event streams for real-time metrics at 10k msg/sec with exactly-once semantics -- HF Engineering\", \"Optimized Spark ETL jobs with partition pruning to cut nightly feature-generation runtime 45% and unblock morning model refreshes -- Johns Hopkins University\"] }]\n" +
    ...
    "13. If the latest user answer directly confirms experience with the technologies you asked about, gives quoted bullets, names target employers/projects, says a technology is skills-only, or gives a paragraph of durable resume context, treat that as enough context and call finish_tailor_resume_interview with USER.md edit operations. Do not ask another placement, wording, permission, or \"should I also insert these into bullets\" question. Never ask whether to add, replace, append, insert, or swap a bullet. Step 3 planning chooses skills versus existing bullets versus appended bullets from your USER.md notes.\n" +
    ...
    "26. When finishing, learnings must be a compact working summary for the next model stage, not a transcript dump. Only include details grounded in the user's answers or directly restated from the accepted plan.\n" +
    ...
    "28. If the latest user answer asks you a question or asks for a sample/example/draft/review, answer it only if the answer is necessary to capture accurate memory. For technology examples, answer with technologyContexts cards plus concise assistantMessage text. If the user also asks to end, finish immediately after saving whatever durable context they provided.\n" +
    "29. Call finish_tailor_resume_interview when the final compressed learnings are ready, when the latest answer directly confirms enough technology context, or when the user asks to end. The tool call ends Step 2 and starts the remaining tailoring steps.\n" +
    "30. When calling finish_tailor_resume_interview, completionMessage should be a short status, not a question. Do not ask the user to confirm USER.md edits, press Done, keep chatting, or approve bullet placement. Do not ask whether to add, replace, append, insert, or swap resume bullets. Do not ask \"do you want me to update USER.md\" or \"should I proceed\" after the user supplied usable memory; write USER.md and finish. Do not add extra normal assistant text beyond that same concise completion.\n" +
    "31. Step 2 records confirmed support and explicit rejections; it does not need to prove every job keyword is usable before finishing. If a high-priority keyword is still unsupported after the user's answer, leave it unsupported so Step 3 does not invent experience.\n" +
    "32. If the user says an emphasized keyword is not a real requirement keyword, is nonsense, or should not be used for this role, call update_tailor_resume_non_technologies and remove it from the emphasized keyword list with a keywordDecisions entry using action \"remove\" and a brief reason. Only do this when the user explicitly rejects the keyword.\n" +
    "33. If no questions are worth asking on the first turn, call skip_tailor_resume_interview instead of starting a chat.\n" +
    "34. Do not add debugDecision or other debug-only fields to interview tools.\n\n" +
    ...
    "3. After each user answer, compare the full conversation against the current USER.md. If the latest answer added durable facts about experience, lack of experience, preferences, constraints, or reusable resume context that are not already in USER.md, include all new durable facts in the USER.md patch when you call finish_tailor_resume_interview. If the user gives a paragraph, quoted bullet text, employer/project mapping, or skills-only constraint, that is explicit authorization to save memory now. If you still need one more concrete factual detail, ask first and save the cumulative facts when finishing; do not ask permission merely to update USER.md.\n" +
