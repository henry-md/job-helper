...
function buildTailorResumePlanningTechnologyContextBlock() {
  return (
    ...
    "- Skills-entry gate: the Skills or Technical Skills section is for actual skills, not for every keyword used to pepper the resume. Add a new skills-list entry only when it is a concrete tool, language, framework, database, infrastructure tool, developer tool, or named method, and either the source resume already supports it as a real skill or USER.md has a dedicated sentence/bullet for that exact technology saying it can be listed in skills, describing direct experience/exposure, or providing quoted experience evidence. When a concrete actual skill passes this gate and you use it in an experience/project bullet, include it in Skills too when an editable Skills block exists. Do not add capability phrases merely for ATS coverage, such as broad API, infrastructure, fundamentals, or process wording, unless USER.md explicitly says that exact phrase can be listed as a skill. Those terms may still appear naturally in experience bullets or keyword coverage checks when they are already provided as job keywords.\n" +
...
const defaultSystemPromptSettings = {
  ...
  tailorResumePlanning:
    ...
    "- For extracted technology keywords, distinguish Skills entries from bullet-only keyword coverage. Concrete technologies with dedicated source-resume or USER.md support may go into Skills under the closest existing category; capability phrases used to pepper fit should not be added to Skills unless USER.md explicitly records them as skills-list entries.\n\n" +
    "Reason rules:\n" +
