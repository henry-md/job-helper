...
function normalizeTailoringWarning(value: string) {
  return value.replace(/^Step [45]:\s*/i, "").trim();
}
...
function buildTailoringWarningMessage(input: {
  ...
  if (normalizedError === sameLineCountWarning) {
    return `Saved a tailored draft for ${input.jobLabel}, but the page-fit check couldn't reduce the page size.`;
  }

  if (normalizedError === pageCountComparisonWarning) {
    return `Saved a tailored draft for ${input.jobLabel}, but the page-fit check couldn't compare the page size.`;
  }
